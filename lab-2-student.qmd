...
surveys <- read_csv(file = here::here("data", "surveys.csv"), show_col_types = FALSE)
```

**2. What are the dimensions (# of rows and columns) of these data?**

```{r}
#| label: finding the dimensions
#| echo: true
## Use the dim function to find the dimensions of surveys
dim(surveys)
```
There are 30463 rows in this data frame and 15 columns.

**3. What are the data types of the variables in this dataset?**

```{r}
#| label: finding the data types
#| echo: true
## https://stackoverflow.com/questions/21125222/determine-the-data-types-of-a-data-frames-columns
## Use the str function to obtain the types of variables
str(surveys)
```
The numeric variables are: record_id, month, day, year, plot_id, hindfoot_length, and weight
The character variables are: species_id, sex, day_of_week, plot_type, genus, species, and taxa
The one variable that is of the type "date" is date 
...
#| label: scatterplot
#| echo: true
## Creating a scatterplot of surveys comparing weight by hindfoot length
ggplot(data = surveys,
       mapping = aes(
         x = weight, 
         y = hindfoot_length)) +
  ...
  facet_wrap(~ species) + 
  labs(title = "Weight of a Rodent Compared to its Hindfoot Length by Species", 
       x = "Weight (g)", 
       y = NULL, 
...
## Creating boxplots of the weights of different species
ggplot(data = surveys,
       mapping = aes(
         x = species,
         y = weight
...
#| echo: true
## Making boxplots that compare the weights of different species, but with a horizontal orientation
ggplot(data = surveys,
       mapping = aes(
         x = species,
         y = weight
...
## Using the aov function to test out our hypotheses
...
Based on our p-value from the ANOVA F-test, which is 2e-16, which is a very small p-value, we have strong evidence against the null hypothesis that the population mean weight is the same across all 14 rodent species, and thus we can reject the null. 
